...
export class getTableDataPage {
  ...
  constructor(page: Page) {
    ...
    this.cardNumberFrame = page.frameLocator("//iframe[@title='Secure card payment input frame']");
    this.cardExpiryFrame = page.frameLocator("//iframe[@title='Secure card payment input frame']");
    this.cardCvcFrame = page.frameLocator("//iframe[@title='Secure card payment input frame']");
    this.productPB1136RB = page.locator("//input[@id='productPB-1136']")
    this.transactionSuccessfulMsg = page.locator("//*[@id='thankYou']")
    this.missingLastName = page.locator("//span[@class='small sc-dnqmqq hrkLxW']")
  }
...
async selectCountrySpecificSubscription() {
  ...
  async validateMissingLastNameAlert() {
    try {
            await expect(this.missingLastName).toBeVisible({ timeout: 6000 });
            let text = await this.missingLastName.textContent();
            return text;
        } catch (error) {
            logger.info(`Element ${this.missingLastName} not found or not visible: `, error);
            console.error(`Element ${this.missingLastName} not found or not visible: `, error);
        }
  }

    async getTableData() {
        const table = await this.page.$('table'); // Get the table element
        if (!table) {
            throw new Error('Table element not found');
        }
        
        const rows = await table.$$('tr'); // Use the `table` variable to get rows

        const tableData: string[][] = [];
        for (const row of rows) {
            const cells = await row.$$('td'); // Get the cells from each row
            const rowData: string[] = [];
            for (const cell of cells) {
                const text = await cell.textContent(); // Get text content from each cell
                rowData.push(text?.trim() || ''); // Handle null textContent
            }
            tableData.push(rowData); // Add the row data to tableData
        }
        return tableData; // Return the collected table data
    }
