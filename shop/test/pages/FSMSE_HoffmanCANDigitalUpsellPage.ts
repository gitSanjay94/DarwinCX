...
export class FSMSE_HoffmanCANDigitalUpsellPage {
  ...
  async validateDigitalArchiveAccessText(): Promise<boolean> {
    try {
      await this.digitalArchiveText.waitFor({ state: 'attached', timeout: 30000 });
       await expect(this.digitalArchiveText).toBeVisible({ timeout: 60000 });
      await this.digitalArchiveText.scrollIntoViewIfNeeded();
