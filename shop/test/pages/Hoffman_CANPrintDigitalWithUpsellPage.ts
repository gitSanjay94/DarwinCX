...
export class Hoffman_CANPrintDigitalWithUpsellPage {
  ...
  readonly productPB1005: Locator;
  ...
  constructor(page: Page) {
    ...
    this.productPB1005 = page.locator("//input[@id='productPB-1005']")
  ...
  async validateUserProvinceValue() {
    await this.userProvince.waitFor({ state: 'visible', timeout: 60000  });
  ...
  async validateUserCountryValue() {
    await this.userCountry.waitFor({ state: 'visible' , timeout: 60000 });
  ...
  async fillMailingAndBillingAddressCA() {
   ...
   async fillMailingAddressCA() {
    await expect(this.userFirstNameValue).toBeVisible({ timeout: 60000 });
    await this.mailingFirstNameTF.scrollIntoViewIfNeeded();
    await this.mailingFirstNameTF.fill('userFirstName' + faker.person.firstName());
    await this.mailingLastNameTF.fill('userLastName' + faker.person.lastName());
    await this.mailingAddressTF.fill('123 Garratt Blvd');
    await this.page.getByText('123 Garratt Blvd, North York, ON, Canada').click();
    await this.page.waitForTimeout(6000);
    await this.mailingAddress2TF.fill('45');
    await this.mailingPhoneNumberTF.fill('1357930055');
    await this.mailingEmailTF.fill(faker.internet.exampleEmail());
  }

  async selectCanadianInternationalOfferToggle() {
    await this.CanadianInternationaOffersToggle.waitFor({ state: 'visible', timeout: 60000  });
  ...
  async selectCANOfferPrintOnly() {
   ...
   async selectCANOfferDigitalOnly() {
    await expect(this.productPB1005).toBeVisible({ timeout: 60000 });
    await this.productPB1005.scrollIntoViewIfNeeded();
    await this.productPB1005.click();
