...
export class CSC_NewparamsinurlUSnoStatePage {
  ...
  constructor(page: Page) {
    ...
    this.missingState = page.locator('span.small.sc-gipzik.kNcaGv', {hasText: 'Select a Province/State'});
  }
...
async validateMissingCityAlert() {
  ...
  async validateMissingZipCodeAlert() {
   ...
   async validateMissingStateAlert() {
    try {
            await expect(this.missingState).toBeVisible({ timeout: 6000 });
            let text = await this.missingState.textContent();
            return text;
        } catch (error) {
            logger.info(`Element ${this.missingState} not found or not visible: `, error);
            console.error(`Element ${this.missingState} not found or not visible: `, error);
        }
  }
