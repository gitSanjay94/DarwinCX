...
export class Upsells_PopupwithUpsellPage {
  ...
  readonly selectButtonBundleFirst: Locator;
  readonly selectButtonBundleSecond: Locator;
  readonly summaryBundleUSDTxtValue: Locator;
  ...
  constructor(page: Page) {
   ...
   this.cardNumber = page.locator("//input[@id='Field-numberInput']");
    ...
    this.selectButtonBundleFirst = page.getByRole('heading', { name: 'Select' }).first();
    this.selectButtonBundleSecond = page.getByRole('heading', { name: 'Select' }).nth(1);
    ...
    this.UpsellPopup = page.getByRole('heading', { name: 'Your Special Offers' })
    // this.summaryBundleUSDTxtValue = page.locator('div').filter({ hasText: /^TotalUSD\$10\.00$/ }).first()
    this.summaryBundleUSDTxtValue = page.locator("//div[normalize-space(text())='USD$10.00']").first();
...
async validateAutorenewCheckbox() {
  ...
  async clickSelectButtonBundleFirst() {
    
    try {
      // await this.page.waitForSelector('//div[@class="sc-ckVGcZ gqdfJF"]', { timeout: 6000 });
      await expect(this.selectButtonBundleFirst).toBeVisible({ timeout: 10000 });
      await this.selectButtonBundleFirst.click()
      } catch (error) {
      logger.info(`Element ${this.selectButtonBundleFirst} not found or not visible: `, error);
      console.error(`Element ${this.selectButtonBundleFirst} not found or not visible: `, error);
    }
   
    }

    async clickSelectButtonBundleSecond() {
    
    try {
      await expect(this.selectButtonBundleSecond).toBeVisible({ timeout: 10000 });
      await this.selectButtonBundleSecond.click()
      } catch (error) {
      logger.info(`Element ${this.selectButtonBundleSecond} not found or not visible: `, error);
      console.error(`Element ${this.selectButtonBundleSecond} not found or not visible: `, error);
    }
  ...
  async fillMailingAndBillingAddress() {
   ...
   async fillMailingAndBillingAddressCA() {
    ...
    async validateUpgradeYourSubscriptionNotVisible(): Promise<boolean> {
    try {
      // Check if the element is not visible within the given timeout
      await expect(this.upgradeYourSubscriptionText).toBeHidden({ timeout: 10000 });
      return true;  // Element is hidden as expected
    } catch (error) {
      // Handle and log the error if the element is visible or not found
      logger.info(`Element ${this.upgradeYourSubscriptionText} not hidden or encountered an error:`, error);
      console.error(`Element ${this.upgradeYourSubscriptionText} not hidden or encountered an error:`, error);
      return false;  // Return false if the element is visible or an error occurred
    }
  }

   async validateUpsellPopup() {
    await this.UpsellPopup.scrollIntoViewIfNeeded();
    await expect(this.UpsellPopup).toBeVisible({ timeout: 80000 });
    const text = await this.UpsellPopup.innerText();
    console.log('Your Special Offers', text); // Debugging statement
    return text.trim();
  }

  async validateBundleCartSummaryUSDTxtValue() {

            await this.page.waitForSelector('//div[@class="sc-ckVGcZ gqdfJF"]', { timeout: 6000 });
            await expect(this.summaryBundleUSDTxtValue).toBeVisible({ timeout: 6000 });

            // Fetch the text content
            const summaryBundleUSDTxtValue = await this.summaryBundleUSDTxtValue.textContent();
            console.log(`Summary USD Txt Value: ${summaryBundleUSDTxtValue}`); // Debug output
            
            // Perform the assertion
            // Return the value for further use if necessary
            return summaryBundleUSDTxtValue?.trim();
