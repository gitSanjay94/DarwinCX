...
export class Courage_CAErrMsgPage {
  ...
  readonly missingFirstNameErrMsg: Locator;
  readonly missingLastNameErrMsg: Locator;
  readonly missingAddressErrMsg: Locator;
  readonly missingCityErrMsg: Locator;
  readonly missingPostalCodeErrMsg: Locator;
  readonly missingProvinceErrMsg: Locator;
  readonly missingEmailErrMsg: Locator; 
  
  readonly emptyFirstNameErrMsg: Locator;
  readonly emptyLastNameErrMsg: Locator;
  readonly emptyAddressErrMsg: Locator;
  readonly emptyCityErrMsg: Locator;
  readonly emptyPostalCodeErrMsg: Locator;
  readonly emptyEmailErrMsg: Locator;
  ...
  constructor(page: Page) {
    ...
    this.transactionSuccessfulMsg = page.locator("//*[@id='thankYou']")
        ...
        this.summaryUSDTxtValue = page.locator('div.col-6 div.row >> text="USD$11.00"');


        this.missingFirstNameErrMsg = page.getByText('First Name is missing')
        this.missingLastNameErrMsg  = page.getByText('Last Name is missing')
        this.missingAddressErrMsg   = page.getByText('required', { exact: true })
        this.missingCityErrMsg      = page.getByText('City is missing')
        this.missingPostalCodeErrMsg= page.getByText('Postal code is missing')
        this.missingProvinceErrMsg  = page.getByText('Select a Province/State')
        this.missingEmailErrMsg     =  page.getByText('Email address is missing')

        this.emptyFirstNameErrMsg = page.getByText('Field cannot be empty')
        this.emptyLastNameErrMsg  = page.getByText('Field cannot be empty').nth(1)
        this.emptyAddressErrMsg   = page.getByText('Field cannot be empty').nth(2)
        this.emptyCityErrMsg      = page.getByText('Field cannot be empty').nth(3)
        this.emptyPostalCodeErrMsg= page.getByText('Field cannot be empty').nth(4)
        this.emptyEmailErrMsg     =  page.getByText('Invalid email address')
  }
...
async fillGiftRecipientMailingAddressCA() {
    ...
    async checkMailingAddressDifferentCB() {
     ...
     async validateMailingAddressTxt() {
       ...
       async validateCartSummaryTxt() {
        try {
            await expect(this.summaryTxt).toBeVisible({ timeout: 6000 });
            let text = await this.summaryTxt.textContent();
            return text;
        } catch (error) {
            logger.info(`Element ${this.summaryTxt} not found or not visible: `, error);
            console.error(`Element ${this.summaryTxt} not found or not visible: `, error);
        }
    }
    ...
    async validateFirstNameMissingText() {
        await expect(this.missingFirstNameErrMsg).toBeVisible({ timeout: 60000 });
        let text = await this.missingFirstNameErrMsg.textContent();
        return text;
               }

    async validateLastNameMissingText() {
        await expect(this.missingLastNameErrMsg).toBeVisible({ timeout: 60000 });
        let text = await this.missingLastNameErrMsg.textContent();
        return text;
               }
    async validateAddressMissingText() {
        await expect(this.missingAddressErrMsg).toBeVisible({ timeout: 60000 });
        let text = await this.missingAddressErrMsg.textContent();
        return text;
    }
    ...
    async validatePostalCodeMissingText() {
        await expect(this.missingPostalCodeErrMsg).toBeVisible({ timeout: 60000 });
        let text = await this.missingPostalCodeErrMsg.textContent();
        return text;
               }

    async validateProvinceMissingText() {
        await expect(this.missingProvinceErrMsg).toBeVisible({ timeout: 60000 });
        let text = await this.missingProvinceErrMsg.textContent();
        return text;
               }

    async validateEmailMissingText() {
        await expect(this.missingEmailErrMsg).toBeVisible({ timeout: 60000 });
        let text = await this.missingEmailErrMsg.textContent();
        return text;
               }

    async validateFirstNameEmptyErrText() {

        await this.page.getByPlaceholder('First Name*').click();
        await this.page.getByPlaceholder('First Name*').fill(' ');
        await expect(this.emptyFirstNameErrMsg).toBeVisible({ timeout: 60000 });
        let text = await this.emptyFirstNameErrMsg.textContent();
        return text;
               }

    async validateLastNameEmptyErrText() {

        await this.page.getByPlaceholder('Last Name*').click();
        await this.page.getByPlaceholder('Last Name*').fill(' ');
        await expect(this.emptyLastNameErrMsg).toBeVisible({ timeout: 60000 });
        let text = await this.emptyLastNameErrMsg.textContent();
        return text;
               }

    async validateAddressEmptyErrText() {

        await this.page.getByPlaceholder('Ex. 123 Example St').click();
        await this.page.getByPlaceholder('Ex. 123 Example St').fill(' ');
        await expect(this.emptyAddressErrMsg).toBeVisible({ timeout: 60000 });
        let text = await this.emptyAddressErrMsg.textContent();
        return text;
               }
    async validateCityEmptyErrText() {

        await this.page.getByPlaceholder('City*').click();
        await this.page.getByPlaceholder('City*').fill(' ');
        await expect(this.emptyCityErrMsg).toBeVisible({ timeout: 60000 });
        let text = await this.emptyCityErrMsg.textContent();
        return text;
               }
        
    async validatePostalCodeEmptyErrText() {

        await this.page.getByPlaceholder('Zip/Postal Code*').click();
        await this.page.getByPlaceholder('Zip/Postal Code*').fill(' ');
        await expect(this.emptyPostalCodeErrMsg).toBeVisible({ timeout: 60000 });
        let text = await this.emptyPostalCodeErrMsg.textContent();
        return text;
               }

     async validateEmailEmptyErrText() {

        await this.page.getByPlaceholder('Email*').click();
        await this.page.getByPlaceholder('Email*').fill(' ');

        await expect(this.emptyEmailErrMsg).toBeVisible({ timeout: 60000 });
        let text = await this.emptyEmailErrMsg.textContent();
        return text;
               }
