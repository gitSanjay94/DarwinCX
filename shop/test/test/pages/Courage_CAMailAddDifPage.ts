...
export class Courage_CAMailAddDifPage {
  ...
  readonly autoRenewMySubscriptionText: Locator;
  readonly purchaserAutoRenewOptionCB: Locator;
  ...
  constructor(page: Page) {
    ...
    this.autoRenewMySubscriptionText = page.getByText('Auto Renew my subscription')
    this.purchaserAutoRenewOptionCB = page.locator('#purchaserAutoRenewOption')
  ...
  async validateAutoRenewMySubscriptionText() {
    try {
      // Check if the element is visible within the specified timeout
      const isVisible = await this.autoRenewMySubscriptionText.isVisible({ timeout: 60000 });
      // If the element is visible, scroll into view and get its text content
      if (isVisible) {
        await this.autoRenewMySubscriptionText.scrollIntoViewIfNeeded();
        let text = await this.autoRenewMySubscriptionText.textContent();
        console.log('Retrieved text content:', text);
        return { isVisible: true, text: text };
      } else {
        // If the element is not visible, return isVisible as false
        console.log('Auto Renew My Subscription text is not visible.');
        return { isVisible: false, text: null };
      }
    } catch (error) {
      // Log the error if something goes wrong
      logger.error('Error occurred while checking auto-renew subscription text visibility: ', error);
      throw error; // Re-throw the error to ensure proper test failure
    }
  }


  async purchaserAutoRenewOptionCheckBox() {
    await expect(this.purchaserAutoRenewOptionCB).toBeVisible({ timeout: 60000 });
    await this.purchaserAutoRenewOptionCB.scrollIntoViewIfNeeded();
    await this.purchaserAutoRenewOptionCB.check();
  }
