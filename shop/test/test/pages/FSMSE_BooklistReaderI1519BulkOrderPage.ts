...
export class FSMSE_BooklistReaderI1519BulkOrderPage {
  ...
  readonly productPB1040: Locator;
  readonly productPB1058: Locator;
  readonly IrelandText: Locator;
  readonly productPB1377: Locator;
  ...
  constructor(page: Page) {
    ...
    this.productPB1040 = page.locator("label[for='productPB-1040']")
    this.productPB1058 = page.locator("label[for='productPB-1058']")
    this.productPB1377 = page.locator("label[for='productPB-1377']")
    ...
    this.IrelandText = page.getByText('CONNECT WITH ALL THE WONDER AND BEAUTY THAT IRELAND HAS TO OFFER')
 
  }


async selectOffers() {
        await expect(this.page.locator('#PB-1377')).toBeVisible({ timeout: 60000 });
        await this.page.locator('#PB-1377').selectOption('17');
  ...
  async selectCountrySpecificSubscription() {
    ...
    await this.productPB1040.click();
    this.page.waitForTimeout(2000)

  }
  async selectInternationlSpecificSubscription() {
    this.page.waitForTimeout(2000)
    await this.productPB1058.click();
    this.page.waitForTimeout(2000)
  ...
  async fillMailingAndBillingAddressCA() {
    ...
    await this.userPostalCodeValue.fill('M5G 0A8');
    await this.userPhoneNumber.fill('1234567890');
    await this.userEmailValue.fill(faker.internet.exampleEmail());
  }
...
async fillMailingAddress() {
    ...
    // await this.mailingCompanyNameTF.fill('mailCompanyName' + faker.company.name());
    // await this.mailingJobTitleTF.fill('Data Scientist');
    await this.mailingAddressTF.fill('123 William St')
    await this.page.getByText('123 William Street, New York, NY, USA').click();
    await this.page.waitForTimeout(6000);
    await this.mailingAddress2TF.fill('CourageCA1Difa32charsforT216Page');
    await this.mailingPhoneNumberTF.fill('2345676903');
    await this.mailingEmailTF.fill(faker.internet.exampleEmail());
   ...
   async fillMailingAndBillingAddressIE() {
    await expect(this.userFirstNameValue).toBeVisible({ timeout: 60000 });
    await this.userFirstNameValue.scrollIntoViewIfNeeded();
    await this.userFirstNameValue.fill('userFirstName' + faker.person.firstName());
    await this.userLastNameValue.fill('userLastName' + faker.person.lastName());
    // await this.companyValue.fill('userCompanyName' + faker.company.name());
    // await this.jobTitileValue.fill('Automation Engineer');
    await this.userAddress1Value.fill('1 W Pier');
    await this.page.getByText('1 West Pier, Howth, Dublin, Ireland').click();
    await this.userAddress2Value.fill('CourageCA1Gift32charsforT216Page');
    await this.userPhoneNumber.fill('1234567890');
    await this.userEmailValue.fill(faker.internet.exampleEmail());
  }
  ...
  async validatIrelandText() {
        await expect(this.IrelandText).toBeVisible({ timeout: 60000 });
        await this.IrelandText.scrollIntoViewIfNeeded();
        let text = await this.IrelandText.textContent();
        return text;
    }
