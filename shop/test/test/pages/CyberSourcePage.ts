...
export class CyberSourcePage {
  ...
  constructor(page: Page) {
    ...
    this.cardNumber = page.locator("//input[@id='number']");
    this.cardExpiry = page.locator("//input[@id='expYear']");
    this.cardCvc = page.locator("//input[@id='securityCode']");
    this.cardNumberFrame = page.frameLocator("//iframe[@title='secure payment field']").first();
    this.cardExpiryFrame = page.frameLocator("//iframe[@title='secure payment field']").first();
    this.cardCvcFrame = page.frameLocator("//iframe[@title='secure payment field']").first();
...
this.emailContent =  page.frameLocator('iframe[name="html_msg_body"]').locator('text=Thank you for your paid membership to B.A.S.S.')
  ...
  async enterPaymentDetail() {
    // Wait for the card number field to be attached to the DOM
    await this.page.frameLocator('#number-container iframe[title="secure payment field"]').getByPlaceholder('Enter card number').click();
    await this.page.frameLocator('#number-container iframe[title="secure payment field"]').getByPlaceholder('Enter card number').fill('4242');
    await this.page.frameLocator('#number-container iframe[title="secure payment field"]').getByPlaceholder('Enter card number').click();
    await this.page.frameLocator('#number-container iframe[title="secure payment field"]').getByPlaceholder('Enter card number').fill('4242 4242 4242 4242');
    await this.page.frameLocator('#securityCode-container iframe[title="secure payment field"]').getByPlaceholder('...').click();
    await this.page.frameLocator('#securityCode-container iframe[title="secure payment field"]').getByPlaceholder('...').fill('123');
    await this.page.getByLabel('Expiry year').selectOption('2026');
    await this.page.getByLabel('Expiry month').selectOption('05');
  ...
  async verifyEmailSentToSender(generatedEmail: string) {
    try {
      ...
      await this.page.getByRole('cell', { name: 'Thank you for your B.A.S.S membership' }).click();
