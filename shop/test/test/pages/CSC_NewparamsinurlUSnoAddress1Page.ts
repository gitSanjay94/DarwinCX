...
export class CSC_NewparamsinurlUSnoAddress1Page {
  page: Page;
  ...
  constructor(page: Page) {
    ...
    this.missingAddress1 = page.locator("//span[@class='small sc-ksYbfQ iQaTpO']");
  }
...
async validateMissingAddress1Alert() {
    try {
            await expect(this.missingAddress1).toBeVisible({ timeout: 6000 });
            let text = await this.missingAddress1.textContent();
            return text;
        } catch (error) {
            logger.info(`Element ${this.missingAddress1} not found or not visible: `, error);
            console.error(`Element ${this.missingAddress1} not found or not visible: `, error);
        }
  }
