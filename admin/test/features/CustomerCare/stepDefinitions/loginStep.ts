...
Given('User opens Darwin User role', async function () {
    try {
...
When('Login to Darwin User role with valid credentials', async function () {
    this.loginP = await this.pomManager.getLoginPage();
...
Then('Verify user should logged in successfully', async function () {
    try {
        ...
        await expect(this.page).toHaveURL(process.env.BASEURL + "/#/dashboard");
        expect(await this.loginP.getAppTitle()).toEqual("Dashboard >< Darwin CX")
