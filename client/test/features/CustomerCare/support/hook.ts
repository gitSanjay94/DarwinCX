...
// check process.env.npm_config_urlToCall for a value if so set it to urlToCall
let urlToCall = process.env.npm_config_urlToCall;

/**
 * BeforeAll() is used to create the Browser before all test scenarios
 */
BeforeAll({ timeout: 150000 }, async function () {
    console.log("Started executing Test Suite BeforeAll")
    if (urlToCall) {
 ...
 */
After({ timeout: 150000 }, async function ({ pickle, result }) {
...
/**
 * AfterAll() is used to close the Browser instance after all test scenarios
 */
